...
def draw_table(args:list,col_width:int):
    style = [
        ...
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),  # 第二行到最后一行左右左对齐
...
def demo():
    ...
    # 添加标题
    content.append(draw_title('数据分析就业薪资'))

    # 添加图片
    content.append(draw_img('抗疫必胜.png'))
    ...
    content.append(draw_little_title('不同级别的平均薪资'))
    # 添加表格
    data = [
        ('职位名称', '平均薪资', '较上年增长率'),
        ('数据分析师', '18.5K', '25%'),
    ...
    # 生成图表
    content.append(draw_title(''))
    content.append(draw_little_title('热门城市的就业情况'))
...
def generatepdf(table:list,name:str,path:str):
    doc = SimpleDocTemplate(path+'/'+name+'.pdf', pagesize=letter)
    doc.build(table)
