...
def xgboost(path):
    ...
    Y_test = Y_2[:96]
    Y_pred = Y_2[96:192]
    pd.DataFrame(Y_test).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_XGBoost_test.xlsx'), index=False, header=False)
    pd.DataFrame(Y_pred).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_XGBoost_pred.xlsx'), index=False, header=False)
