...
def lstm_procasting(df):
    ...
    train_size = int(len(scaled_data) - 193)
    ...
    # 训练模型
    model.fit(X_train, y_train, batch_size=64, epochs=1)     # batch_size=64
    ...
    def for_predict(data, model, pre_lengths, dim):
        ...
        if dim == 1:
            for i in range(pre_lengths):
                ...
                init_sequence = init_sequence.reshape(1, 96, dim)
                result.append(pred)
    ...
    dim = 1
    pre_lengths = 96
    init_sequence = scaled_data[-97:-1].reshape(1, 96, dim)
