...
def train(train_loader):
    ...
    torch.save(model, 'transformer_96.pt')
...
def evaluate(test_loader, max_price, min_price):
    model = torch.load('transformer_96.pt')
    model.eval()
    preds = []
    reals = []
    # labels=[]
    ...
    #plt.legend()
    #plt.savefig("pred_96.png", dpi=300)
    ...
    pd.DataFrame(preds).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_Transformer_test.xlsx'), index=False, header=False)
...
def predict(train_data, pred_data, max_price, min_price):
    ...
    pd.DataFrame(preds).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_Transformer_pred.xlsx'), index=False, header=False)
