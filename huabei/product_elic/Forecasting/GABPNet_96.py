...
def gabpnet(path):
    ...
    b=a[:96]
    c=a[96:192]
    pd.DataFrame(b).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_GABPNet_test.xlsx'), index=False, header=False)
    pd.DataFrame(c).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/96_GABPNet_pred.xlsx'), index=False, header=False)
#    # 显示测试图像
