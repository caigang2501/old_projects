...
def gabpnet(path):
    # 要打开的文件名
    df = pd.read_excel(path, usecols=['load', 'temperature', 'wind_speed', 'price'])
    # 数据预处理
    ...
    # 分训练和测试集
    x_train, x_test, y_train, y_test = train_test_split(x_norm, y_norm, test_size=192, shuffle = False)
    ...
    a=list(pre_org.reshape(192))
    b=a[:96]
    c=a[96:192]
    pd.DataFrame(b).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm96_GABPNet_test.xlsx'), index=False, header=False)
    pd.DataFrame(c).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm96_GABPNet_pred.xlsx'), index=False, header=False)
#    # 显示测试图像
