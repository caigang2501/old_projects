...
def getData(all_data, sequence_length, batch_size):
    ...
    train_x, train_y = x[:-96, ], y[:-96, ]
    # 最后96组作为测试数据
    test_x, test_y = x[-96:, ], y[-96:, ]
...
def train(train_loader):
    ...
    torch.save(model, 'transformer_asfm96.pt')
...
def evaluate(test_loader, max_price, min_price):
    model = torch.load('transformer_asfm96.pt')
    model.eval()
    preds = []
    reals = []
    # labels=[]
    ...
    plt.legend()
    plt.savefig("pred_asfm96.png", dpi=300)
    ...
    pd.DataFrame(preds).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm96_Transformer_test.xlsx'), index=False, header=False)
...
def predict(train_data, pred_data, max_price, min_price):
    ...
    pd.DataFrame(preds).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm96_Transformer_pred.xlsx'), index=False, header=False)

def transformer(data_path):
    data = pd.read_excel(data_path, usecols=['data', 'load', 'temperature', 'wind_speed', 'price'])
