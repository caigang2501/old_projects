...
# if __name__ == '__main__':
    ...
    Y = df['price']
    Y = Y.drop(Y.tail(10).index)
    X = df.drop(['price'],axis=1)
    X = X.drop(X.tail(10).index)
    ...
    Y_test = Y_2[:5]
    Y_pred = Y_2[5:10]
    pd.DataFrame(Y_test).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm_XGBoost_test.xlsx'), index=False, header=False)
    pd.DataFrame(Y_pred).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm_XGBoost_pred.xlsx'), index=False, header=False)
