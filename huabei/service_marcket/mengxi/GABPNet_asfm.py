...
def gabpnet(path):
    ...
    # 分训练和测试集
    x_train, x_test, y_train, y_test = train_test_split(x_norm, y_norm, test_size=10, shuffle = False)
    ...
    a=list(pre_org.reshape(10))
    b=a[:5]
    c=a[5:10]
    pd.DataFrame(b).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm_GABPNet_test.xlsx'), index=False, header=False)
    pd.DataFrame(c).to_excel(os.path.join(os.path.dirname(os.path.abspath(__file__)),'result/asfm_GABPNet_pred.xlsx'), index=False, header=False)
#    # 显示测试图像
